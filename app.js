...
function renderDashboard() {
  ...
  document.querySelector('#upcomingSchedule').innerHTML=events.length ? events.map(t=>`<div class="schedule-row schedule-item" data-task-id="${t.id}"><span class="schedule-date">${t.date.slice(5).replace('-','/')}</span><strong>${escapeHTML(t.title)}</strong><small>${t.startTime?`${t.startTime}${t.endTime?` – ${t.endTime}`:''}`:'未设时间'}</small></div>`).join(''):'<p class="empty-copy">暂时没有接下来的待办。</p>';
