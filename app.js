...
function renderDashboard() {
  ...
  const activeDeadlines=data.deadlines.filter(d=>new Date(d.end)>new Date()); document.querySelector('#deadlineList').innerHTML = activeDeadlines.length ? activeDeadlines.sort((a,b)=>new Date(a.end)-new Date(b.end)).map(d=>`<article class="deadline-card"><div class="deadline-row"><span class="countdown">${countdown(d.end)}</span><strong>${escapeHTML(d.title)}</strong><small>${d.start.slice(5,16).replace('T',' ').replace('-','/')} – ${d.end.slice(5,16).replace('T',' ').replace('-','/')}</small></div></article>`).join('') : '<p class="empty-copy">暂时没有进行中的重要时间段。</p>';
  const events=data.tasks.filter(t=>{if(!t.date||!t.startTime||t.done)return false;const finalTime=t.endTime||t.startTime;return new Date(`${t.date}T${finalTime}`)>new Date();}).sort((a,b)=>`${a.date}${a.startTime}`.localeCompare(`${b.date}${b.startTime}`));
  document.querySelector('#upcomingSchedule').innerHTML=events.length ? events.map(t=>`<div class="schedule-row schedule-item" data-task-id="${t.id}"><span class="schedule-date">${t.date.slice(5).replace('-','/')}</span><strong>${escapeHTML(t.title)}</strong><small>${t.startTime}${t.endTime?` – ${t.endTime}`:''}</small></div>`).join(''):'<p class="empty-copy">带上时间的待办会出现在这里。</p>';
...
function renderCalendar() {
  ...
  for(let i=0;i<42;i++){ const current=new Date(start);current.setDate(start.getDate()+i);const dKey=toDateKey(current), inMonth=current.getMonth()===m;
    ...
    grid.push(`<div class="calendar-day ${inMonth?'':'outside'} ${dKey===todayKey?'today':''}" data-date="${dKey}" title="点击空白处添加事项"><div class="day-number"><b>${current.getDate()}</b>${dKey===todayKey?'<em>今</em>':''}</div>${tasks.filter(t=>!t.startTime).slice(0,2).map(t=>`<div class="calendar-task calendar-item ${t.done?'done':''}" data-task-id="${t.id}"><input class="calendar-check" type="checkbox" data-id="${t.id}" ${t.done?'checked':''}/><span>${escapeHTML(t.title)}</span></div>`).join('')}${tasks.filter(t=>t.startTime).slice(0,2).map(t=>`<div class="calendar-event calendar-item ${t.done?'done':''}" data-task-id="${t.id}"><input class="calendar-check" type="checkbox" data-id="${t.id}" ${t.done?'checked':''}/><span>${t.startTime} ${escapeHTML(t.title)}</span></div>`).join('')}${deadlines.slice(0,1).map(d=>`<div class="calendar-deadline">${escapeHTML(d.title)}</div>`).join('')}</div>`);
...
function renderJournal() {
  ...
  document.querySelector('#journalWeekday').textContent = journalDateKey === todayKey ? '今日手记' : '往日手记';
...
const modal=document.querySelector('#taskModal'), form=document.querySelector('#taskForm');let editingTaskId=null;let editingTaskBucket='recent';
...
function openModal(kind='task', quick='', task=null, bucket='recent') { form.reset();editingTaskId=task?.id||null;editingTaskBucket=task?taskBucket(task):bucket;form.itemType.value=kind;document.querySelector('#taskModalTitle').textContent=task?'编辑事项':'添加一个事项';form.querySelector('button[type="submit"]').textContent=task?'保存修改':'保存事项';document.querySelectorAll('.choice-card').forEach(x=>x.classList.toggle('active',x.querySelector('input').value===kind));if(task){form.title.value=task.title;form.date.value=task.date;form.startTime.value=task.startTime;form.endTime.value=task.endTime;form.notes.value=task.notes||'';}syncModalFields();if(quick==='today')form.date.value=todayKey;else if(quick)form.date.value=quick;openLayer(modal);form.title.focus();}
