...
function deadlineToneClass(deadline) { return `deadline-tone-${Math.max(0, data.deadlines.indexOf(deadline)) % 7}`; }
...
function renderCalendar() {
  ...
  for(let i=0;i<42;i++){
    ...
    const deadlineSegments=deadlines.map(d=>{const startKey=d.start.slice(0,10),endKey=d.end.slice(0,10),lane=deadlineLanes.get(d.id),startHour=Number(d.start.slice(11,13)),endHour=Number(d.end.slice(11,13)),startTime=d.start.slice(11,16),endTime=d.end.slice(11,16);const segmentStart=dKey===startKey||weekday===0,segmentEnd=dKey===endKey||weekday===6,startHalf=dKey===startKey&&startHour>12,endHalf=dKey===endKey&&endHour<=12;const label=segmentStart?`${dKey===startKey?`${startTime} `:''}${escapeHTML(d.title)}`:'';return `<div class="calendar-deadline ${deadlineToneClass(d)} ${segmentStart?'deadline-segment-start':''} ${segmentEnd?'deadline-segment-end':''} ${startHalf?'deadline-start-half':''} ${endHalf?'deadline-end-half':''}" data-deadline-id="${d.id}" style="--deadline-lane:${lane}" title="单击编辑时间段">${label?`<span class="deadline-label">${label}</span>`:''}${dKey===endKey?`<span class="deadline-end-time">${endTime}</span>`:''}</div>`;}).join('');
